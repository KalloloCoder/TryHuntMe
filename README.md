...
- **Stop Server** — gracefully terminate the running vulnerable server.  
...
```
usage: tryhuntme [-h] [-v] {start-server,stop-server,recon,poc,report,about} ...

TryHuntMe — educational bug hunting toolkit (local only).

positional arguments:
  {start-server,stop-server,recon,poc,report,about}
    start-server        Start the local vulnerable server for testing
    stop-server         Stop the running vulnerable server
...
- Stop running server:
```bash
python3 tryhuntme.py stop-server
```

- Recon module:
```bash
python3 tryhuntme.py recon -t 192.168.1.10 --start 20 --end 1000
```

- Generate PoC payload:
```bash
python3 tryhuntme.py poc --type xss --target http://example.com --obfuscate
```

- Generate a markdown report:
```bash
python3 tryhuntme.py report --target example.com -o myreport.md
```
...
[KalloloCoder](https://github.com/KalloloCoder) ([Muh.Agus Tri Ananda](https://www.instagram.com/agstriananda)) — Creator & Maintainer.  
