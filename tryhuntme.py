...
def start_vuln_server(port=8000, open_browser=False):    
    ...
    # Run vuln_server.py in a subprocess so user can still use CLI in another terminal    
    path = os.path.join(os.path.dirname(__file__), "vuln_server.py")    
    if not os.path.exists(path):    
        colorful_print("[!] vuln_server.py not found!", Fore.RED)    
        return    
    # Use Popen to start server    
    if sys.platform.startswith("win"):    
        Popen([sys.executable, path, str(port)], creationflags=0)    
    else:    
        Popen([sys.executable, path, str(port)])    
